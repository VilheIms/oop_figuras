...
public class OIzveide {
	...
	static void izveidotObjektu() {
		...
		switch(izvele) {
		case 0: x = MinkuTante.skaitlaParbaude("Ievadi centra punkta x koordinātas",-100, 100);
		...
		case 1:
			...
			String veids = (String)JOptionPane.showInputDialog(null, "Izvēles, kuru konstruktoru izsaukt", "Četrstūra izveide", JOptionPane.QUESTION_MESSAGE, null, ceIzvelesVeids, ceIzvelesVeids[0]);
			if(veids == null)
			...
			case "Centra punkts un malas":
				cPNr = cPIzvele();
				if (cPNr == -1) {
					JOptionPane.showMessageDialog(null, "Netika izvēlēts centra punkts ko izvēlēties!", "Kļūda", JOptionPane.ERROR_MESSAGE);
					break;
			}
		...
		case 2:
			...
			if(r == -1) {
				JOptionPane.showMessageDialog(null, "Aplis netika izveidots!", "Brīdinājums", JOptionPane.WARNING_MESSAGE);
			}
		...
		if(m1 == -1 || m2 == -1 || m3 == -1) {
				JOptionPane.showMessageDialog(null, "Trijtūris netika izveidots!", "Brīdinājums", JOptionPane.WARNING_MESSAGE);
			}
		...
		break;
		case 4: JOptionPane.showMessageDialog(null, "Atgriežas uz galveno izvēlni!", "Paziņojums", JOptionPane.INFORMATION_MESSAGE);
		break;
